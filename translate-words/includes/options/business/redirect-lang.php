...
class Redirect_Lang extends Abstract_Boolean {
	...
	protected function get_description(): string {
		return sprintf(
			...
			__( 'Remove the page name or page ID from the URL of the front page: %1$s to remove, %2$s to keep.', 'translate-words' ),
			'`true`',
			'`false`'
		);
	}
	...
	public function get_site_health_info( Options $options ): array { // phpcs:ignore VariableAnalysis.CodeAnalysis.VariableAnalysis.UnusedVariable
		if ( $this->get() ) {
			$value = '1: ' . __( 'The front page URL contains the language code instead of the page name or page id', 'translate-words' );
		} else {
			$value = '0: ' . __( 'The front page URL contains the page name or page id instead of the language code', 'translate-words' );
		}
