		...
		if ( doing_action( 'create_term' ) && in_array( $taxonomy, $this->get_taxonomies_to_copy( true ) ) ) {
			...
			$tr_posts = get_posts(
				array(
					...
					'post_status' => 'any',
					'fields'      => 'ids',
					// phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_tax_query -- This tax_query is required to fetch all posts linked to the created term and its translations, so we can associate the new term with all relevant translated posts. This is essential for correct multilingual term syncing; no performant or reliable alternative exists in core WP for this logic.
					'tax_query'   => array(
						array(
							'taxonomy'         => $taxonomy,
							'field'            => 'id',
